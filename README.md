...
- 예금은 가입 건별 3턴 약정·고정 금리를 보관합니다. 중도해지는 발생 이자의 50%만 지급하는 가상 약정이며 원금 차감이 없습니다. 만기에 IRP 대기자금으로 전환하고 디폴트옵션 대상 자금으로 기록합니다.
- 위험등급은 1(매우 높은 위험) → 6(매우 낮은 위험) 방향의 가상 예시입니다. 규제상 위험자산 분류와 기초 주식 노출을 분리합니다. 적격 TDF 예외는 손실 위험이 없다는 뜻이 아닙니다.
- 12턴은 압축 체험이며 납입·공제 한도는 1년분입니다. 디폴트옵션은 만기 2턴 대기·통지 후 1턴으로 압축해 자동 적용하며, 직접 옵트인·옵트아웃·현금 유지 지시와 구분합니다.
...
- ⑧ 자동운용 상태: 자금별 통지·대기, 만기/신규가입/일반 추가납입 구분, 고정 구성의 가상 승인형 편입분, 옵트인·옵트아웃·현금 유지 지시.
- ⑦ 성향: 5성향별 시작/목표 구성과 생활자금·낙폭 기준. 캐릭터 외형은 독립 선택, v7 저장으로 기존 기록 보존.
- 효과음: 비동기 오디오 활성화 완료 후 재생, 중단 상태 복구·음소거 시 예약음 취소, 설정에 효과음 테스트 추가.

최신 규칙과 가정은 사용자·운영자 매뉴얼의 `step-b` 절을 참고하세요. 기존 감사 자료 및 P0 구현 기록은 해당 버전 당시 기준입니다.
