...
const pacingGuide=`<section id="step-f"><h1>연금로드 v1.6.0 · 납입 속도와 운용 화면</h1>
...
for(const file of ['public/user-manual.html','public/operator-manual.html']) {
  ...
  writeFileSync(file,head+`<body><main style="max-width:1080px;margin:auto;padding:24px">${pacingGuide}${tradeGuide}${guide}<details id="legacy-manual"><summary>v1.4 이하 이전 설명 · 당시 저장 게임 참고용</summary><p>아래의 디폴트옵션 자동매수·구성·저장 안내는 새 판에 적용하지 않습니다. 현재 규칙은 위의 현재 버전 안내를 참고하세요.</p><!-- legacy-content:start -->${legacy}<!-- legacy-content:end --></details></main></body></html>`);
}
