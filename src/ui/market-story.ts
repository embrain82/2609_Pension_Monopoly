...
export type MarketStoryLedger = Pick<TurnLedger, 'open' | 'afterMarket' | 'marketEffects' | 'cashInterest'>;

/** 시장 구간의 기록만 표시한다. 현재 잔액·납입액으로 과거 손익을 역산하지 않는다. */
export function renderIrpApplied(ledger: Pick<TurnLedger, 'open' | 'afterMarket' | 'cashInterest'>, final = false): string {
  ...
  return `<p class="news-irp ${tone}"><span>내 IRP에 반영 · ${ledger.cashInterest?'시장 손익·대기 이자':'시장 손익'}</span><b>${delta > 0 ? '+' : ''}${formatWon(delta)}</b><small>(${signedPercent(rate)})</small><em>이미 잔고에 들어갔어요 · ${final ? '마지막 주문은 추가 시장 없이 최종 정산합니다' : '지금 행동은 다음 턴에 걸립니다'}</em></p>`;
}
...
export function renderMarketStory(raw: MarketStep, ledger?: MarketStoryLedger, characters = false): string {
  ...
  return `<div class="market-story" aria-label="시장 변화의 원인과 내 보유분 영향">
    ...
    <section class="market-story-step market-story-holdings"><h3><span aria-hidden="true">03</span> 내 보유분에 반영</h3>${ledger ? renderIrpApplied(ledger,step.turn===12) : '<p class="hint">이전 저장에는 이 턴의 시장 손익 합계가 없습니다.</p>'}${renderMarketImpacts(ledger?.marketEffects,step.turn,false,true)}${renderCashInterest(ledger?.cashInterest)}<p class="story-basis">시장 구간만 비교합니다. 추가납입·매매 결과는 포함하지 않습니다.</p></section>
  </div>`;
}
