...
/** 도감 「캐릭터 컬렉션」 탭. 선택 가능한 캐릭터 5종, 완주 수, 최고 별 */
export function renderCollectionGallery(collection: Collection, characters: boolean): string {
  const items = PROFILE_IDS.map((id) => {
    ...
    return `<article class="collection-card ${done ? 'earned' : 'locked'}">
        ${characters ? renderAvatar(id, entry.bestStars >= 2 ? 'happy' : done ? 'calm' : 'tense', 56) : `<b class="collection-initial">${AVATAR_NAMES[id].slice(0, 1)}</b>`}
        <h3>${AVATAR_NAMES[id]}<small>모든 투자성향에서 선택 가능</small></h3>
