...
export function validDefaultLedger(g: Omit<GameState,'campaign'>): boolean {
  ...
  if(!['2026-09-10-e','2026-09-15-f','2026-09-16-g'].includes(g.rulesetVersion)) return !d && g.holdings.every(h=>!h.positions) && g.pendingOrders.every(o=>!o.defaultScope);
  if(!d||d.version!=='e1'||!Array.isArray(d.groups)||d.groups.length>(g.rulesetVersion==='2026-09-16-g'?256:24)) return false;
