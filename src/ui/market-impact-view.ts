...
export function renderMarketImpacts(effects: MarketHoldingEffect[] | undefined, turn: number, chart = true, compact = false): string {
  ...
  const rows = main.map(effect => {
    ...
    return `<li><div><strong>${name}</strong><b class="${effect.delta < 0 ? 'neg' : 'pos'}">${won(effect.delta)} <small>(${effect.returnRate > 0 ? '+' : ''}${(effect.returnRate * 100).toFixed(2)}%)</small></b></div>${compact ? '' : `<small>${effect.productId === 'deposit' ? '가입 건별 약정·만기 반영 · 시장 예시와 다름' : '실제 가격 변동·상품 보수 반영'}</small>`}</li>`;
  }).join('');
  return `<div class="preview-box actual-market-impact"><strong>내 보유분에 실제 반영 · 주요 ${Math.min(2, effects.length)}개</strong>${bars}<ul>${rows}</ul><small>${compact ? '시장 구간 · 보수·예금 약정·가격 변동 중 주문 반영' : '턴 시작 시장 구간 · 매매·납입 전. 가격 변동 중인 대기 주문 포함.'}</small></div>`;
}
