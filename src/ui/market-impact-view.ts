...
export function renderMarketImpacts(effects: MarketHoldingEffect[] | undefined, turn: number): string {
  if (turn === 0) return '';
  ...
  const rows = [...effects].sort((a, b) => Math.abs(b.delta) - Math.abs(a.delta)).slice(0, 2).map(effect => {
  ...
  return `<div class="preview-box actual-market-impact"><strong>내 보유분에 실제 반영 · 주요 ${Math.min(2, effects.length)}개</strong><ul>${rows}</ul><small>턴 시작 시장 구간 · 매매·납입 전. 가격 변동 중인 대기 주문 포함.</small></div>`;
}
