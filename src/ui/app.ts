...
import { defaultScopes, defaultValue } from '../engine/position-engine';
import { renderDefaultTrade } from './default-trade-view';
...
import { REGIONS, reflectRegion } from '../engine/route-engine';
...
import { finalQuizCards, optionalQuizCards } from '../engine/quiz-engine';
...
import { renderAchievementGallery, renderCollectionGallery, renderLifeResolvedStrip, renderSeedLine, renderWeeklyButton } from './achievements-view';
import { rebalanceShares, rebalancePortfolio, buyProduct, sellProduct, switchProduct } from '../engine/portfolio-engine';
import { canBuyForProfile, expectedRiskAfterBuy, type BuyLimitDecision } from '../engine/policy-engine';
...
export class PensionRoadApp {
  ...
  private preparation: StartPreparation | null = null;
  private renderedScreenMark = '';
  private renderedActionView: ActionView = 'menu';
  private modal: Modal = null;
  private pendingPrompt: PendingPrompt | null = null;
  private uiProgress: UiProgress = {};
  private restorePromptPosition: { scroll: number; focusAction?: string } | null = null;
   ...
   * 업적은 두 경우 모두 다시 평가해 새로 연 것만 더한다(연금으로 바꾸면 「연금 수령 체험」이 열릴 수 있다).
   */
  ...
  private scheduleAutoSettle(summary: TurnSummary): void {
    ...
    this.autoSettleTimer = window.setTimeout(() => {
      ...
      if (this.game && quizOpportunity(this.game).maxPoints > 0) { this.render(); return; }
  ...
  private onClick(event: Event): void {
    ...
    if (!action) return;
    if (this.pendingPrompt && !['notice-continue', 'notice-quiz', 'close-modal'].includes(action)) return;
    ...
    if (action === 'notice-continue') {
    ...
    } else if (action === 'begin' && this.canStart()) {
      this.beginPreparation(randomSeed());
      ...
      this.preparation.stage = 'option';
    } else if (action === 'prepare-back' && this.preparation) {
      this.preparation.stage = 'profile'; this.screen = 'prepare';
      ...
      if(replay) { this.resumeGame({version:checkpointVersion(replay),game:replay,modal:null,lastSummary:null,quizCardId:null,quizPicked:null,finalQuizQueue:[],finalQuizTotal:0,finishing:false,defaultOptionAsk:false}); this.render(); }
      return;
    } else if (action === 'action-quiz' && this.game) {
      const card=this.game.learningFlow?optionalQuizCards(this.game)[0]:this.game.pendingQuizCardId;
      if(card)this.openQuiz(card);
      ...
      this.render(); this.root.querySelector<HTMLButtonElement>(`[data-action="cards-tab"][data-tab="${this.cardsTab}"]`)?.focus({preventScroll:true}); return;
    } else if (action === 'answer') {
      if (this.screen !== 'diagnosis' || Number(button.dataset.question) !== this.questionIndex) return;
      this.diagnosisScore += Number(button.dataset.score ?? 0);
      this.questionIndex += 1;
      if (this.questionIndex >= questions.length) {
        if (this.preparation) {
          this.preparation = reassessPreparation(this.preparation, profileFromScore(this.diagnosisScore));
          this.screen = 'prepare'; this.render(); return;
        }
        this.profileId = profileFromScore(this.diagnosisScore);
        this.save.profileId = this.profileId;
        this.save.profileAssessment = { profileId: this.profileId, origin: 'diagnosed' };
        if (this.game) {
      ...
      if (view === 'default') this.initializeDefaultTrade();
      this.buyLimitConfirm = null;
      ...
      if (isDefaultOptionId(button.dataset.option)) {
        if (this.preparation) { this.preparation.option = button.dataset.option; this.preparation.optionSource='chosen'; }
        else this.defaultOptionPick = button.dataset.option;
      }
    } else if (action === 'confirm-default-option') {
      if (this.preparation) this.confirmPreparation();
      else this.applyDefaultOptionChoice(this.defaultOptionPick);
    } else if (action === 'skip-default-option') {
      this.applyDefaultOptionChoice(null);
    } else if (action === 'open-default-option') {
      if (!this.game && !this.save.profileAssessment) { this.beginPreparation(randomSeed()); this.render(); return; }
      ...
      else if((view==='sell'||view==='switch') && actionAvailability(this.game,view).enabled) { this.selectedSell='deposit'; this.switchFrom='deposit'; this.actionView=view; }
      else return;
      this.portfolioReturn=false; this.amountPreset='max'; this.actionScrollTop=null; this.requestActionEntry();
    } else if (action === 'return-action') {
      this.portfolioReturn = false; this.requestActionEntry();
  ...
  private progressQuizOpportunity(intent: ProgressIntent) {
    if (!this.game) return { cardIds: [], nextPoints: 0, maxPoints: 0 };
    if (intent.kind === 'skip-quiz') return quizOpportunity(this.game, [intent.cardId]);
    return quizOpportunity(this.game, this.game.learningFlow && this.game.turn > 0 ? optionalQuizCards(this.game) : []);
  }

  private canProgress(intent: ProgressIntent): boolean {
    if (!this.game || this.screen !== 'game') return false;
    if (intent.kind === 'enter-action') return this.game.awaitingAction && !this.game.currentEventId;
  ...
  private requestProgress(intent: ProgressIntent): void {
    if (this.pendingPrompt || !this.game || !this.canProgress(intent)) return;
    this.clearAutoSettle();
    const risk = intent.kind === 'enter-action' && marketRiskNotice(this.game).exceeded && this.uiProgress.riskNoticeAckTurn !== this.game.turn;
    const opportunity = intent.kind === 'enter-action' ? null : this.progressQuizOpportunity(intent);
    const quiz = opportunity && opportunity.maxPoints > 0 && !quizOpportunityAcknowledged(this.uiProgress, this.game.turn, opportunity.cardIds);
    if (risk || quiz) {
      const active = document.activeElement as HTMLElement | null;
      this.pendingPrompt = { kind: risk ? 'risk-notice' : 'quiz-confirm', intent,
        returnModal: risk ? null : this.modal,
        scroll: this.root.querySelector<HTMLElement>('.modal-sheet')?.scrollTop ?? 0,
        focusAction: active?.dataset.action };
      this.modal = this.pendingPrompt.kind;
      return;
    }
    this.executeProgress(intent);
  }

  private executeProgress(intent: ProgressIntent): void {
    if (!this.canProgress(intent)) return;
    if (intent.kind === 'enter-action') this.modal = 'action';
    else if (intent.kind === 'skip-quiz') this.applyQuizSkip();
    else if (intent.kind === 'leave-settlement') this.applySettlementExit();
    else void this.beginDiceRoll();
  }

  private continueNotice(): void {
    const prompt = this.pendingPrompt;
    if (!prompt || !this.game) return;
    if (prompt.kind === 'risk-notice') this.uiProgress.riskNoticeAckTurn = this.game.turn;
    else {
      const ids = this.progressQuizOpportunity(prompt.intent).cardIds;
      const previous = this.uiProgress.quizSkipAck?.turn === this.game.turn ? this.uiProgress.quizSkipAck.cardIds : [];
      this.uiProgress.quizSkipAck = { turn: this.game.turn, cardIds: [...new Set([...previous, ...ids])] };
    }
    this.pendingPrompt = null; this.modal = prompt.returnModal;
    this.executeProgress(prompt.intent);
  }

  private returnToNoticeQuiz(): void {
    const prompt = this.pendingPrompt;
    if (!prompt || prompt.kind !== 'quiz-confirm') return;
    const card = this.progressQuizOpportunity(prompt.intent).cardIds[0];
    this.pendingPrompt = null;
    if (card) this.openQuiz(card);
    else { this.modal = prompt.returnModal; this.executeProgress(prompt.intent); }
  }

  private cancelNotice(): void {
    const prompt = this.pendingPrompt;
    if (!prompt) return;
    this.pendingPrompt = null; this.modal = prompt.returnModal;
    this.restorePromptPosition = prompt.returnModal ? { scroll: prompt.scroll, focusAction: prompt.focusAction } : null;
    this.announce(prompt.kind === 'risk-notice' ? '보드로 돌아왔습니다. 운용 횟수는 그대로입니다.' : '원래 화면으로 돌아왔습니다. 턴과 점수는 그대로입니다.');
  }
  ...
  private closeModal(): void {
    if (this.pendingPrompt) { this.cancelNotice(); return; }
    if (this.modalIsSticky()) return;
    this.regionNotice = "";
    if (this.modal === 'default-option') { this.modal = 'settings'; return; }
    if(this.modal==='quiz' && this.game?.learningFlow) { if (this.quizPicked === null) this.skipQuiz(); else this.nextQuiz(); return; }
    if (this.modal === 'action') { this.buyLimitConfirm = null; this.modal = null; this.announce('운용 선택을 취소했습니다. 턴과 남은 행동은 그대로입니다.'); }
    else if (this.modal === 'portfolio' && this.portfolioReturn) { this.portfolioReturn = false; this.requestActionEntry(); }
  ...
  private resolveLife(choice: LifeChoice | undefined): void {
    ...
    if (this.game.awaitingAction) this.requestActionEntry(); else this.modal = null;
    this.persist(true);
  }
  ...
  private afterMarketScene(): void {
    ...
    if (!this.game.learningFlow && this.game.pendingQuizCardId && !this.game.quizLog.some((record) => record.cardId === this.game!.pendingQuizCardId)) {
      this.openQuiz(this.game.pendingQuizCardId);
      return;
    }
    if (this.game.currentEventId) this.modal = 'life';
    else if (this.game.awaitingAction) this.requestActionEntry();
    else this.modal = null;
  }
  ...
  private pickQuiz(option: number): void {
    ...
    const beforePoints = knowledgeScoreOf(this.game);
  ...
  /** 건너뛰기: 새 판은 해당 문항을 이번 판에서 제외하고, 구 판은 기존 마무리 후보 규칙을 유지한다. */
  private skipQuiz(): void {
    if (!this.game || !this.quizCardId) return;
    if (this.quizPicked !== null || this.game.quizLog.some(q => q.cardId === this.quizCardId)) { this.nextQuiz(); return; }
    this.requestProgress({ kind: 'skip-quiz', cardId: this.quizCardId });
  }

  private applyQuizSkip(): void {
    if (!this.game) return;
    if (this.game.pendingQuizCardId === this.quizCardId) this.game = { ...this.game, pendingQuizCardId: null };
    if(this.game.learningFlow)this.game={...this.game,learningFlow:{...this.game.learningFlow,queue:this.game.learningFlow.queue.filter(id=>id!==this.quizCardId),dismissed:[...new Set([...(this.game.learningFlow.dismissed??[]),...(this.quizCardId?[this.quizCardId]:[])])]}};
  ...
  private nextQuiz(): void {
    ...
    else this.modal = this.lastSummary ? 'settle' : null;
  }

  /** 정산 창 닫기. 새 판은 수령으로, 구 판은 기존 마무리 퀴즈를 이어간다. */
  private afterSettlement(): void {
    this.requestProgress({ kind: 'leave-settlement' });
  }

  private applySettlementExit(): void {
    this.clearAutoSettle();
    if (!this.game || this.game.status !== 'finished') { this.modal = null; return; }
    this.finishing = true;
    if(this.game.learningFlow) { this.quizCardId=null; this.quizPicked=null; this.finalQuizQueue=[]; this.finalQuizTotal=0; this.modal='payout'; return; }
  ...
  private runAction(action: GameAction): void {
    ...
    // 12턴도 정산을 먼저 표시한다. 새 판의 선택 학습은 이 화면에서 열며, 이후 수령 비교로 간다.
  ...
  private async moveRolledDice(): Promise<void> {
    ...
    for (const [i, position] of path.entries()) {
      if (!this.motion.valid(id)) return;
      if (instant && i < path.length - 1) continue;
  ...
  private resumeGame(data: PlayCheckpoint): void {
    ...
    if (this.modal === 'action') this.requestActionEntry();
    this.announce('저장된 결정 시점에서 이어갑니다. 시장·급여·거래를 다시 처리하지 않습니다.');
  }

  private initializeDefaultTrade(): void {
    if(!this.game)return;
    ...
    } else if(!tabs[this.defaultTradeDraft.tab].enabled && (tabs.in.enabled||tabs.out.enabled)) this.defaultTradeDraft.tab=tabs.in.enabled?'in':'out';
  }
  ...
  private cancelPreparation(): void {
    ...
    this.screen = this.preparation.returnTo; this.preparation = null; this.modal = null;
    this.announce('시작 준비를 닫았습니다. 기존 진행은 그대로입니다.');
  }

  private confirmPreparation(): void {
    const d = this.preparation;
    if (!d || !canConfirmPreparation(d)) return;
    this.profileId = d.profileId; this.scenarioId = d.scenario; this.missionId = d.mission; this.goalMonthly = d.goal;
    if (d.provenance !== 'weekly') {
      this.save.profileId = d.profileId;
  ...
  private startGame(seed: string): void {
    ...
    this.diceRolling = false;
    this.tokenHopping = false;
    this.tokenFocus = 0; this.tokenTrail = []; this.regionNotice = "";
    this.finishing = false;
    this.finalQuizQueue = [];
    this.quizCardId = null;
    this.quizPicked = null;
  ...
  private onKeydown(event: KeyboardEvent): void {
    if (!this.root.isConnected) return;
    const galleryTab = event.target instanceof Element ? event.target.closest('[data-action="cards-tab"]') : null;
    if (this.modal === 'cards' && galleryTab && ['ArrowLeft','ArrowRight','Home','End'].includes(event.key)) {
      event.preventDefault(); const tabs=[...this.root.querySelectorAll<HTMLButtonElement>('[data-action="cards-tab"]')];
      const index=tabs.indexOf(galleryTab as HTMLButtonElement);
    ...
    if (radio && (this.modal === 'default-option' || this.preparation?.stage === 'option') && ['ArrowRight', 'ArrowDown', 'ArrowLeft', 'ArrowUp', 'Home', 'End'].includes(event.key)) {
      ...
      if (isDefaultOptionId(option)) { if(this.preparation) { this.preparation.option=option; this.preparation.optionSource='chosen'; } else this.defaultOptionPick = option; this.render(); this.root.querySelector<HTMLButtonElement>(`[data-action="pick-default-option"][data-option="${option}"]`)?.focus(); }
      return;
    }
    ...
    if (event.key === 'Escape' && this.preparation) { this.cancelPreparation(); this.render(); return; }
  ...
  private render(): void {
    ...
    const screenHtml = this.screen === 'prepare' && this.preparation ? renderStartPreparation(this.preparation)
      : this.screen === 'title' ? this.renderTitle()
    ...
    if (dialog) {
      ...
      if (previousModal !== this.modal || !dialog.contains(document.activeElement)) {
    ...
    if (dialog && this.restorePromptPosition) {
      ...
      [...dialog.querySelectorAll<HTMLElement>('[data-action]')].find(el => el.dataset.action === restore.focusAction)?.focus({ preventScroll: true });
      dialog.scrollTop = restore.scroll; this.restorePromptPosition = null;
    }
    const screenMark=`${this.screen}:${this.preparation?.stage??''}:${this.questionIndex}`;
    if(screenMark!==this.renderedScreenMark && (this.screen==='prepare'||this.screen==='diagnosis')) {
      const heading=this.root.querySelector<HTMLElement>('h1');heading?.setAttribute('tabindex','-1');heading?.focus({preventScroll:true});window.scrollTo(0,0);
    }
    if(this.screen!==this.renderedScreenMark.split(':')[0] && ['game','result','title'].includes(this.screen)) {
  ...
  private renderDiagnosis(): string {
    ...
    return `<section class="setup-screen narrow road-diagnosis">
      ...
      <div class="choice-stack">${question.options.map(([label, score], index) => `<button data-action="answer" data-key="question-${this.questionIndex}-${index}" data-question="${this.questionIndex}" data-score="${score}"><span class="choice-index">${String.fromCharCode(65 + index)}</span>${label}<span aria-hidden="true">→</span></button>`).join('')}</div></div>
  ...
  private renderGoal(): string {
    ...
    return `<section class="setup-screen narrow road-goal">
      ${this.game||this.save.profileAssessment?`<div class="profile-stamp">${profile.name}</div>`:''}
      <h1>월 연금 목표를 정하세요</h1>
      <p>${this.game||this.save.profileAssessment?profile.description:'성향은 새 판 시작 전 진단으로 확인합니다.'} 성향은 서열이 아니라 감당 가능한 변동을 확인하는 기준입니다.</p>
      ...
      <div class="game-layout">
        ...
        <aside class="dashboard" tabindex="0" aria-label="시장과 자산 상세">
          ...
          <article class="asset-card"><div class="card-label-row"><div class="card-label">나의 은퇴설계</div>${this.save.settings.characters ? renderAvatar(state.avatarId, avatarMood(state, mission.passed), 44) : ''}</div>
            ...
            <div class="risk-line"><span>위험자산 비중 <b>${riskRatioLabel(score.riskRatio)}</b></span><span>생활자금 ${formatShortWon(state.cash)}</span></div>
            ${renderRiskMeter(score.riskRatio, policyRules.riskAssetLimit)}
            ${marketRiskNotice(state).exceeded ? '<p class="warning risk-limit-warning">시장 가격 변화로 한도 초과 · 위험자산 추가매수 제한. 가능한 행동은 운용 메뉴에서 확인하세요.</p>' : ''}
  ...
  private renderResult(): string {
    ...
    return `<section class="result-screen road-result">
      ...
      <div class="result-actions"><button class="secondary" data-action="same-seed">같은 시드로 다시</button><button class="secondary" data-action="new-seed">새 시드로 도전</button><button class="text-button" data-action="open-cards" data-tab="cards">관련 학습 카드 보기</button><button class="text-button" data-action="to-title">타이틀로</button></div>
    </section>`;
  }
  ...
  private renderModal(): string {
    ...
    if (this.modal === 'risk-notice' && this.game) content = renderRiskNotice(this.game);
    ...
    if (this.modal === 'settle' && this.lastSummary) {
      ...
      content = renderSettlementModal(summary, {
        ...
        reducedMotion,
        optionalLearning: !!this.game?.learningFlow,
    ...
    return `<div class="modal-backdrop"><section class="modal-sheet modal-${this.modal}${['news','life','tile'].includes(this.modal)||this.modal==='action'&&this.actionView==='menu'?' road-scene':''}${this.modal==='action'&&this.actionView==='menu'?' road-action-menu':''}${this.modal==='action'&&this.actionView!=='menu'?' road-order':''}${this.modal==='portfolio'?' road-portfolio':''}${['quiz','settle','payout'].includes(this.modal)?' road-learning road-'+this.modal:''}${['settings','howto','cards','market','explore','default-option'].includes(this.modal)?' road-support':''}" role="dialog" aria-modal="true" aria-label="${labels[this.modal]}"${this.pendingPrompt ? ' aria-labelledby="turn-notice-title" aria-describedby="turn-notice-description"' : ''}>${close ? `<div class="modal-close-bar">${close}</div>` : close}${['settings','howto','cards','market','explore','default-option'].includes(this.modal)?`<header class="support-brand">${brandWordmark()}</header>`:''}${content}<p class="modal-feedback" aria-live="polite">${this.pendingPrompt ? '' : this.feedback}</p>${this.modal === 'action' ? '<footer class="action-footer"><button class="secondary" data-action="action-portfolio">포트폴리오 확인</button><small>확인만으로 행동 횟수가 줄지 않아요.</small></footer>' : ''}</section></div>`;
  }
  ...
  private renderSettlementLearning(): string {
    if (!this.game || (!this.game.campaign && !this.game.learningFlow)) return '';
  ...
  private renderActionModal(): string {
    ...
    if (this.actionView === 'buy') {
      ...
      const pending = game.turn>=11 ? '마지막 구간 거래 · 아래 최종 정산 일정 확인' : isSpotlight
  ...
  private renderPortfolioModal(): string {
    if (!this.game) return '';
    return renderPortfolio(this.game);
  }
  ...
  private renderCardsModal(): string {
    ...
    const nav = `<div class="tab-row" role="tablist" aria-label="도감">${tabs.map(([tab, label]) => `<button type="button" role="tab" id="gallery-tab-${tab}" aria-controls="gallery-panel" tabindex="${this.cardsTab===tab?0:-1}" class="${this.cardsTab === tab ? 'active' : ''}" aria-selected="${this.cardsTab === tab}" data-action="cards-tab" data-tab="${tab}">${label}</button>`).join('')}</div>`;
    if (this.cardsTab === 'achievements') {
      return `<p class="eyebrow">도감</p><h2>업적 ${this.save.achievements.length} / ${ACHIEVEMENT_COUNT}</h2>${nav}<div id="gallery-panel" role="tabpanel" aria-labelledby="gallery-tab-${this.cardsTab}"><p class="hint">판이 끝나면 결과 화면에서 새로 연 업적이 뜹니다. 누적 업적은 여러 판에 걸쳐 열립니다.</p>${renderAchievementGallery(this.save.achievements)}</div>`;
    }
  ...
  private renderSettingsModal(): string {
    ...
    const profileNote = playing
      ? `이번 판 성향은 <strong>${playing.name}</strong>이며 ${playing.minRiskGrade}~6등급 매수할 수 있습니다. 이번 판에서는 성향이 고정됩니다. 다음 새 판의 시작 준비에서 다시 진단할 수 있습니다.`
      : this.save.profileAssessment ? `이전에 확인한 성향은 <strong>${nextProfile?.name}</strong>이며 ${nextProfile?.minRiskGrade}~6등급 매수할 수 있습니다.` : '아직 확인한 투자자성향이 없습니다. 새 판의 시작 준비에서 먼저 5문항으로 알아보세요.';
    return `<p class="eyebrow">교육용 게임 안내</p><h2>플레이 설정</h2>
      ...
      <ul class="source-list"><li><a href="${policyRules.source_urls[0]}" target="_blank" rel="noreferrer">국가법령정보센터 · 위험자산 투자한도</a></li><li><a href="${policyRules.source_urls[1]}" target="_blank" rel="noreferrer">소득세법 · 연금계좌 세액공제</a></li><li><a href="https://pension.kebhana.com/files/POR/Notice/PSNL_IRP_PRD_INVTM.pdf" target="_blank" rel="noreferrer">하나은행 · 개인형 IRP 핵심설명서</a></li></ul></details>
      <p class="hint">${this.game ? contributionRuleLabel(this.game) : `새 게임 추가납입 턴당 ${formatShortWon(balanceConfig.contributionPerTurnLimit)}`} · 게임 진행용 한도</p><p class="version">연금로드 v1.10.0 · 성향 진단·시작 위험비중·대기자금 이자·A-1 디자인 · 저장 데이터는 이 브라우저에만 보관됩니다.</p>`;
  }
}
