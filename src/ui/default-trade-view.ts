...
export function renderDefaultHoldings(state: GameState): string {
  ...
  return `<section class="default-positions"><h3>디폴트옵션 운용분</h3>${scopes.length?scopes.map(scope=>`<div class="preview-box"><strong>${defaultPortfolio(scope.optionId).name} · ${won(defaultValue(state,scope))}</strong><p>보유분: ${defaultPortfolio(scope.optionId).products.map(id=>`${products.find(p=>p.id===id)!.shortName} ${won(scopedHolding(state,id,scope).amount)}`).join(' · ')}${state.pendingOrders.some(o=>o.defaultScope?.mandateId===scope.mandateId)?`<br>미결제 주문 ${won(state.pendingOrders.filter(o=>o.defaultScope?.mandateId===scope.mandateId).reduce((sum,o)=>sum+o.amount,0))} · 위 옵션 합계에 포함, 계좌의 미결제 분류와 같은 금액`:''}</p><small>일반 매도·리밸런싱에서 제외됩니다. 디폴트옵션 메뉴에서 묶음으로 환매하세요.</small></div>`).join(''):'<p>보유한 디폴트옵션이 없습니다.</p>'}${renderDefaultOrderStatus(state)}</section>`;
}
export function renderDefaultTrade(state: GameState, draft: DefaultTradeDraft): string {
  ...
  return `<button class="text-button" data-action="action-view" data-view="menu">← 운용지시</button>
    <header class="order-heading"><p class="eyebrow">TURN ${state.turn} · 남은 행동 ${state.actionsLeft}회 · 확정 시 1회</p><h2>디폴트옵션 직접 거래</h2><p>사전지정과 실제 매수·환매는 별개예요.</p></header>
    <div class="default-trade-stats default-account-summary"><div><small>사전지정 · 선호 방법</small><strong>${selection}</strong></div><div><small>현재 보유 · 실제 운용</small><strong>${held?defaultPortfolio(held.optionId).name:'없음'}</strong></div><div><small>디폴트옵션 보유·주문</small><strong>${won(defaultValue(state))}</strong></div><div><small>주문 가능 IRP 대기자금</small><strong>${won(state.irpCash)}</strong></div></div>
    ...
    <div class="order-split"><section class="order-input" aria-labelledby="default-input-title"><h3 id="default-input-title"><span>01</span> ${isIn?'매수할 옵션·금액':'환매할 보유분'}</h3>
    ...
    <p class="hint">${isIn?'지정만으로 매수되지 않습니다. 실제 거래할 옵션과 금액을 확인하세요.':'사전지정 해제나 계좌 밖 인출이 아닙니다.'}</p></section>
    <section class="order-review" aria-labelledby="default-review-title"><h3 id="default-review-title"><span>02</span> 미리보기·확정</h3>
