...
export function renderDefaultTrade(state: GameState, draft: DefaultTradeDraft): string {
  ...
  return `<button class="text-button" data-action="action-view" data-view="menu">← 운용지시</button>
    ...
    <div class="tab-row default-trade-tabs" role="group" aria-label="디폴트옵션 거래 선택"><button class="${isIn?'active':''}" aria-pressed="${isIn}" data-action="default-trade-tab" data-tab="in">직접 매수<small>옵트인</small></button><button class="${!isIn?'active':''}" aria-pressed="${!isIn}" data-action="default-trade-tab" data-tab="out">직접 운용 전환<small>옵트아웃</small></button></div>
    ${isIn?`<label for="default-trade-option">매수할 옵션</label><select id="default-trade-option">${DEFAULT_PORTFOLIOS.map(p=>`<option value="${p.id}" ${p.id===draft.optionId?'selected':''} ${!allowed.has(p.id)||(held&&held.optionId!==p.id)?'disabled':''}>${p.name} · 가상 ${p.riskGrade}등급${!allowed.has(p.id)?' · 성향 밖':held&&held.optionId!==p.id?' · 보유 옵션 환매 후 선택':''}</option>`).join('')}</select><p>${option.blurb}</p><label for="default-trade-amount">매수 금액 (원)</label><input id="default-trade-amount" type="number" inputmode="numeric" min="100000" step="1" max="${Math.floor(state.irpCash)}" value="${draft.amount}"><div class="amount-presets"><button data-action="default-trade-amount" data-fraction="0.5">대기자금 절반</button><button data-action="default-trade-amount" data-fraction="1">대기자금 전액</button></div>`:
    `<p>디폴트옵션 구성품을 같은 비율로 환매합니다. 직접 매수한 같은 상품은 유지됩니다.</p><div class="amount-presets"><button class="${draft.fraction===.5?'active':''}" data-action="default-trade-fraction" data-fraction="0.5">보유분 50%</button><button class="${draft.fraction===1?'active':''}" data-action="default-trade-fraction" data-fraction="1">보유분 전부</button></div>`}
